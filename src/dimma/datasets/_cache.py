...
def get_cache_dir(subdir: str = "") -> Path:
    ...
    Resolution order:
    1. If the ``DIMMA_HOME`` environment variable is set, use it.
    2. Otherwise use the OS-appropriate user cache directory:
       - Linux: ``$XDG_CACHE_HOME/dimma`` or ``~/.cache/dimma``
       - macOS: ``~/Library/Caches/dimma``
       - Windows: ``%LOCALAPPDATA%\\dimma\\Cache``
    3. If ``subdir`` is provided, append it to the resolved path.

    The directory (and any subdir) is created via ``mkdir(parents=True,
    exist_ok=True)``. The function never raises if the directory
    already exists.
